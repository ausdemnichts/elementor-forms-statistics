...
function mdp_get_last_conversion_form_id($user_id) {
    $user_id = (int) $user_id;
    if ($user_id <= 0) {
        return '';
    }
    $value = get_user_meta($user_id, 'mdp_efs_last_conversion_form_id', true);
    return is_string($value) ? sanitize_text_field($value) : '';
}

function mdp_set_last_conversion_form_id($user_id, $form_id) {
    $user_id = (int) $user_id;
    if ($user_id <= 0) {
        return;
    }
    ...
    update_user_meta($user_id, 'mdp_efs_last_conversion_form_id', $form_id);
}

function mdp_get_conversion_settings() {
    $defaults = array(
        'consent_tool' => 'auto',
        'ccm19_category' => 'marketing',
        'usercentrics_category' => 'ADVERTISING',
        'complianz_category' => 'marketing',
        'conversion_name' => '',
        'conversion_currency' => 'EUR',
        'email_key' => 'email',
        'name_key' => 'name',
    );
    $stored = get_option('mdp_efs_conversion_settings', []);
    if (!is_array($stored)) {
        $stored = [];
    }
...
function mdp_get_conversion_form_settings() {
    $stored = get_option('mdp_efs_conversion_form_settings', []);
    return is_array($stored) ? $stored : [];
}

function mdp_get_conversion_form_setting($form_id) {
    $form_id = sanitize_text_field($form_id);
    $all = mdp_get_conversion_form_settings();
    $defaults = array(
        'conversion_name' => '',
        'conversion_currency' => 'EUR',
    );
    if ($form_id === '' || !isset($all[$form_id]) || !is_array($all[$form_id])) {
        return $defaults;
    }
    return wp_parse_args($all[$form_id], $defaults);
}

function mdp_detect_consent_tool() {
    ...
    foreach ($plugins as $plugin_path => $plugin_data) {
        ...
        if ($name && stripos($name, 'ccm19') !== false) {
            return 'ccm19';
        }
        if ($name && stripos($name, 'usercentrics') !== false) {
            return 'usercentrics';
        }
        if ($name && stripos($name, 'complianz') !== false) {
            return 'complianz';
        }
        if ($name && stripos($name, 'erecht24') !== false) {
            return 'usercentrics';
        }
        if ($plugin_path === 'complianz-gdpr/complianz-gdpr.php' || $plugin_path === 'complianz-gdpr-premium/complianz-gdpr-premium.php') {
            return 'complianz';
        }
        if ($plugin_path === 'erecht24-3/erecht24-3.php' || $plugin_path === 'erecht24/erecht24.php') {
            return 'usercentrics';
        }
        if ($plugin_path === 'ccm19/ccm19.php') {
            return 'ccm19';
        }
    }
    return 'none';
...
function mdp_resolve_consent_tool($settings = null) {
    $settings = $settings ? $settings : mdp_get_conversion_settings();
    $selected = isset($settings['consent_tool']) ? sanitize_text_field($settings['consent_tool']) : 'auto';
    if ($selected === 'auto') {
        return mdp_detect_consent_tool();
    }
    return $selected;
}

function mdp_get_consent_tool_labels() {
    return array(
        'auto' => __('Automatisch erkennen', 'elementor-forms-statistics'),
        'ccm19' => __('CCM19', 'elementor-forms-statistics'),
        'usercentrics' => __('Usercentrics', 'elementor-forms-statistics'),
        'complianz' => __('Complianz', 'elementor-forms-statistics'),
        'none' => __('Kein Consent-Tool', 'elementor-forms-statistics'),
    );
}

function mdp_get_conversion_identifier_keys() {
    return array('gclid', 'gbraid', 'wbraid');
}
...
function custom_menu_item() {
    ...
    if (mdp_user_can_access_menu('statistiken-conversions')) {
        add_submenu_page(
            ...
            __('Conversions', 'elementor-forms-statistics'),
            'edit_posts',
            'statistiken-conversions',
            'mdp_conversions_page_callback'
        );
    }
}
...
add_action('admin_post_mdp_save_conversion_settings', 'mdp_save_conversion_settings');
add_action('admin_post_mdp_export_conversions_csv', 'mdp_export_conversions_csv');
...
add_action('wp_ajax_mdp_save_conversion_last_form', 'mdp_save_conversion_last_form_ajax');
add_action('wp_ajax_mdp_toggle_conversion', 'mdp_toggle_conversion_ajax');
add_action('wp_ajax_mdp_save_conversion_value', 'mdp_save_conversion_value_ajax');
add_action('wp_ajax_mdp_save_conversion_time', 'mdp_save_conversion_time_ajax');
add_action('init', 'mdp_schedule_submission_cleanup');
add_action('wp_enqueue_scripts', 'mdp_enqueue_conversion_tracking');
add_action('elementor_pro/forms/new_record', 'mdp_capture_conversion_identifiers', 20, 2);
...
function mdp_save_conversion_last_form_ajax() {
    if (!mdp_user_can_access_export_menu()) {
        wp_send_json_error(array('message' => __('Keine Berechtigung.', 'elementor-forms-statistics')));
    }
    check_ajax_referer('mdp_conversion_settings', 'nonce');
    $form_id = isset($_POST['form_id']) ? sanitize_text_field(wp_unslash($_POST['form_id'])) : '';
    if ($form_id === '') {
        wp_send_json_error(array('message' => __('Kein Formular ausgewählt.', 'elementor-forms-statistics')));
    }
    mdp_set_last_conversion_form_id(get_current_user_id(), $form_id);
    wp_send_json_success();
}

function mdp_toggle_conversion_ajax() {
    if (!mdp_user_can_access_export_menu()) {
        wp_send_json_error(array('message' => __('Keine Berechtigung.', 'elementor-forms-statistics')));
    }
    check_ajax_referer('mdp_conversion_settings', 'nonce');
    ...
    if ($submission_id <= 0) {
        wp_send_json_error(array('message' => __('Ungültige Anfrage.', 'elementor-forms-statistics')));
    }
    if ($enabled) {
        mdp_set_submission_value($submission_id, 'mdp_conversion', '1');
        mdp_set_submission_value($submission_id, 'mdp_conversion_time', current_time('mysql'));
    } else {
        mdp_delete_submission_value($submission_id, 'mdp_conversion');
        mdp_delete_submission_value($submission_id, 'mdp_conversion_time');
    }
    wp_send_json_success();
}

function mdp_save_conversion_value_ajax() {
    if (!mdp_user_can_access_export_menu()) {
        wp_send_json_error(array('message' => __('Keine Berechtigung.', 'elementor-forms-statistics')));
    }
    check_ajax_referer('mdp_conversion_settings', 'nonce');
    ...
    if ($value === '') {
        mdp_delete_submission_value($submission_id, 'mdp_conversion_value');
    } else {
        mdp_set_submission_value($submission_id, 'mdp_conversion_value', $value);
    }
    wp_send_json_success();
}

function mdp_save_conversion_time_ajax() {
    if (!mdp_user_can_access_export_menu()) {
        wp_send_json_error(array('message' => __('Keine Berechtigung.', 'elementor-forms-statistics')));
    }
    check_ajax_referer('mdp_conversion_settings', 'nonce');
    ...
    if ($value === '') {
        mdp_delete_submission_value($submission_id, 'mdp_conversion_time_manual');
    } else {
        mdp_set_submission_value($submission_id, 'mdp_conversion_time_manual', $value);
    }
    wp_send_json_success();
}

function mdp_enqueue_conversion_tracking() {
    if (is_admin()) {
        return;
    }
    $settings = mdp_get_conversion_settings();
    $form_settings = $selected_form_id !== '' ? mdp_get_conversion_form_setting($selected_form_id) : array('conversion_name' => '', 'conversion_currency' => 'EUR');
    $script_path = plugin_dir_path(__FILE__) . 'assets/js/conversions-tracking.js';
    $script_version = file_exists($script_path) ? (string) filemtime($script_path) : '1.0.0';
    wp_enqueue_script(
        'mdp-conversions-tracking',
        plugin_dir_url(__FILE__) . 'assets/js/conversions-tracking.js',
        array(),
        $script_version,
        true
    );
    $config = array(
        'consentTool' => mdp_resolve_consent_tool($settings),
        'categories' => array(
            'ccm19' => $settings['ccm19_category'],
            'usercentrics' => $settings['usercentrics_category'],
            'complianz' => $settings['complianz_category'],
        ),
    );
    wp_add_inline_script('mdp-conversions-tracking', 'window.mdpConversionTracking = ' . wp_json_encode($config) . ';', 'before');
}

function mdp_capture_conversion_identifiers($record, $ajax_handler) {
    $settings = mdp_get_conversion_settings();
    $consent_tool = mdp_resolve_consent_tool($settings);
    $consent_cookie = isset($_COOKIE['mdp_efs_consent']) ? sanitize_text_field(wp_unslash($_COOKIE['mdp_efs_consent'])) : '';
    if ($consent_tool !== 'none' && $consent_cookie !== '1') {
        return;
    }
    $identifiers = array();
    foreach (mdp_get_conversion_identifier_keys() as $key) {
        $cookie_key = 'mdp_' . $key;
    ...
    if (empty($identifiers)) {
        ...
        if ($referrer !== '') {
            ...
            if ($query) {
                ...
                foreach (mdp_get_conversion_identifier_keys() as $key) {
                    if (!empty($params[$key])) {
                        $identifiers[$key] = sanitize_text_field((string) $params[$key]);
                    }
                }
            }
        }
    ...
    if (is_object($ajax_handler) && method_exists($ajax_handler, 'get_current_form')) {
        ...
        if (is_array($current_form) && isset($current_form['id'])) {
            $element_id = sanitize_text_field($current_form['id']);
        }
    }
    if ($element_id === '') {
        return;
    }
    $submission_id = mdp_find_latest_submission_id($element_id);
    if ($submission_id <= 0) {
        return;
    }
    foreach ($identifiers as $key => $value) {
        mdp_set_submission_value($submission_id, $key, $value);
    }
}
...
function mdp_get_conversion_entries($form_id, $limit = 50) {
    global $wpdb;
    $form_id = sanitize_text_field($form_id);
    if ($form_id === '') {
        return [];
    ...
    $settings = mdp_get_conversion_settings();
    $email_key = isset($settings['email_key']) ? sanitize_key($settings['email_key']) : 'email';
    $name_key = isset($settings['name_key']) ? sanitize_key($settings['name_key']) : 'name';
    $limit = max(1, (int) $limit);
    $submissions_table = $wpdb->prefix . 'e_submissions';
    $values_table = $wpdb->prefix . 'e_submissions_values';
    $email_key_sql = esc_sql($email_key);
    $name_key_sql = esc_sql($name_key);
    $sql = $wpdb->prepare(
        "SELECT s.ID,
                s.created_at_gmt,
                MAX(CASE WHEN ev.`key` = 'gclid' THEN ev.value END) AS gclid,
                MAX(CASE WHEN ev.`key` = 'gbraid' THEN ev.value END) AS gbraid,
                MAX(CASE WHEN ev.`key` = 'wbraid' THEN ev.value END) AS wbraid,
                MAX(CASE WHEN ev.`key` = '{$email_key_sql}' THEN ev.value END) AS email,
                MAX(CASE WHEN ev.`key` = '{$name_key_sql}' THEN ev.value END) AS name,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion' THEN ev.value END) AS conversion_flag,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_time' THEN ev.value END) AS conversion_time,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_time_manual' THEN ev.value END) AS conversion_time_manual,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_value' THEN ev.value END) AS conversion_value,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_exported' THEN ev.value END) AS conversion_exported
         FROM {$submissions_table} s
         LEFT JOIN {$values_table} ev ON ev.submission_id = s.ID
         WHERE s.element_id = %s
           AND s.status NOT LIKE %s
        GROUP BY s.ID
        HAVING MAX(CASE WHEN ev.`key` IN ('gclid','gbraid','wbraid') THEN ev.value END) IS NOT NULL
...
function mdp_get_marked_conversions($form_id, $limit = 1000) {
    global $wpdb;
    $form_id = sanitize_text_field($form_id);
    if ($form_id === '') {
        return [];
    ...
    $sql = $wpdb->prepare(
        "SELECT s.ID,
                ...
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_time' THEN ev.value END) AS conversion_time,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_time_manual' THEN ev.value END) AS conversion_time_manual,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_value' THEN ev.value END) AS conversion_override,
                MAX(CASE WHEN ev.`key` = 'mdp_conversion_exported' THEN ev.value END) AS conversion_exported
         FROM {$submissions_table} s
         LEFT JOIN {$values_table} ev ON ev.submission_id = s.ID
         WHERE s.element_id = %s
           AND s.status NOT LIKE %s
         GROUP BY s.ID
         HAVING MAX(CASE WHEN ev.`key` = 'mdp_conversion' THEN ev.value END) = '1'
            AND MAX(CASE WHEN ev.`key` = 'mdp_conversion_value' THEN ev.value END) <> ''
         ORDER BY s.created_at_gmt DESC
         LIMIT %d",
        $form_id,
        '%trash%',
...
function mdp_enqueue_conversions_assets($hook_suffix) {
    if ($hook_suffix !== 'statistiken_page_statistiken-conversions') {
        return;
    }
    $script_path = plugin_dir_path(__FILE__) . 'assets/js/conversions.js';
    $script_version = file_exists($script_path) ? (string) filemtime($script_path) : '1.0.0';
    wp_enqueue_script(
        'mdp-conversions-admin',
        plugin_dir_url(__FILE__) . 'assets/js/conversions.js',
        array('jquery'),
        $script_version,
        true
    );
    $config = array(
        ...
        'nonce' => wp_create_nonce('mdp_conversion_settings'),
        'i18n' => array(
            'saved' => __('Gespeichert', 'elementor-forms-statistics'),
        ),
    );
    wp_add_inline_script('mdp-conversions-admin', 'window.mdpConversionsConfig = ' . wp_json_encode($config) . ';', 'before');
}
add_action('admin_enqueue_scripts', 'mdp_enqueue_conversions_assets');
...
function mdp_save_conversion_settings() {
    if (!mdp_user_can_access_export_menu()) {
        wp_die(__('Sie haben keine Berechtigung, diese Seite zu sehen.', 'elementor-forms-statistics'));
    }
    if (!isset($_POST['mdp_conversion_settings_nonce']) || !wp_verify_nonce($_POST['mdp_conversion_settings_nonce'], 'mdp_save_conversion_settings')) {
        wp_die(__('Ungültige Anfrage.', 'elementor-forms-statistics'));
    }
    $consent_tool = isset($_POST['consent_tool']) ? sanitize_text_field(wp_unslash($_POST['consent_tool'])) : 'auto';
    $tool_choices = array_keys(mdp_get_consent_tool_labels());
    if (!in_array($consent_tool, $tool_choices, true)) {
        $consent_tool = 'auto';
    }
    $settings = array(
        'consent_tool' => $consent_tool,
        'ccm19_category' => isset($_POST['ccm19_category']) ? sanitize_text_field(wp_unslash($_POST['ccm19_category'])) : 'marketing',
        'usercentrics_category' => isset($_POST['usercentrics_category']) ? sanitize_text_field(wp_unslash($_POST['usercentrics_category'])) : 'ADVERTISING',
        'complianz_category' => isset($_POST['complianz_category']) ? sanitize_text_field(wp_unslash($_POST['complianz_category'])) : 'marketing',
        'email_key' => isset($_POST['email_key']) ? sanitize_key(wp_unslash($_POST['email_key'])) : 'email',
        'name_key' => isset($_POST['name_key']) ? sanitize_key(wp_unslash($_POST['name_key'])) : 'name',
    );
    update_option('mdp_efs_conversion_settings', $settings);

    $form_id = isset($_POST['form_id']) ? sanitize_text_field(wp_unslash($_POST['form_id'])) : '';
    if ($form_id !== '') {
        $form_settings = mdp_get_conversion_form_settings();
        $form_settings[$form_id] = array(
            'conversion_name' => isset($_POST['conversion_name']) ? sanitize_text_field(wp_unslash($_POST['conversion_name'])) : '',
            'conversion_currency' => isset($_POST['conversion_currency']) ? sanitize_text_field(wp_unslash($_POST['conversion_currency'])) : 'EUR',
        );
        update_option('mdp_efs_conversion_form_settings', $form_settings);
    }
    $redirect = add_query_arg(
        array(
            'page' => 'statistiken-conversions',
            'mdp_conversion_saved' => 1,
        ),
        admin_url('admin.php')
    );
    wp_safe_redirect($redirect);
...
function mdp_export_conversions_csv() {
    if (!mdp_user_can_access_export_menu()) {
        wp_die(__('Sie haben keine Berechtigung, diese Seite zu sehen.', 'elementor-forms-statistics'));
    }
    if (!isset($_POST['mdp_export_conversions_nonce']) || !wp_verify_nonce($_POST['mdp_export_conversions_nonce'], 'mdp_export_conversions')) {
        wp_die(__('Ungültige Anfrage.', 'elementor-forms-statistics'));
    }
    $form_id = isset($_POST['form_id']) ? sanitize_text_field(wp_unslash($_POST['form_id'])) : '';
    if ($form_id === '') {
    ...
    $settings = mdp_get_conversion_settings();
    $form_settings = mdp_get_conversion_form_setting($form_id);
    $conversion_name = isset($_POST['conversion_name']) ? sanitize_text_field(wp_unslash($_POST['conversion_name'])) : $form_settings['conversion_name'];
    $conversion_currency = isset($_POST['conversion_currency']) ? sanitize_text_field(wp_unslash($_POST['conversion_currency'])) : $form_settings['conversion_currency'];
    if ($conversion_name === '') {
        wp_die(__('Bitte Conversion-Name angeben.', 'elementor-forms-statistics'));
    }
    $form_settings_store = mdp_get_conversion_form_settings();
    $form_settings_store[$form_id] = array(
        'conversion_name' => $conversion_name,
        'conversion_currency' => $conversion_currency,
    );
    update_option('mdp_efs_conversion_form_settings', $form_settings_store);
    $rows = mdp_get_marked_conversions($form_id);
    if (empty($rows)) {
        wp_die(__('Keine markierten Conversions gefunden.', 'elementor-forms-statistics'));
    }
    $has_value = false;
    foreach ($rows as $row) {
        $override_value = isset($row['conversion_override']) ? sanitize_text_field($row['conversion_override']) : '';
        if ($override_value !== '') {
            $has_value = true;
            break;
        }
    ...
    if (!$has_value) {
        wp_die(__('Bitte zuerst Auftragswerte eintragen.', 'elementor-forms-statistics'));
    }
    $filename = 'google-ads-conversions-' . sanitize_file_name($form_id) . '-' . gmdate('Y-m-d') . '.csv';
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename=' . $filename);
    $output = fopen('php://output', 'w');
    fputcsv($output, array('Google Click ID', 'GBRAID', 'WBRAID', 'Conversion Name', 'Conversion Time', 'Conversion Value', 'Conversion Currency'));
    $exported_ids = [];
    foreach ($rows as $row) {
        $gclid = isset($row['gclid']) ? sanitize_text_field($row['gclid']) : '';
        $gbraid = isset($row['gbraid']) ? sanitize_text_field($row['gbraid']) : '';
        $wbraid = isset($row['wbraid']) ? sanitize_text_field($row['wbraid']) : '';
        if ($gclid === '' && $gbraid === '' && $wbraid === '') {
            continue;
        }
        $manual_time = isset($row['conversion_time_manual']) ? sanitize_text_field($row['conversion_time_manual']) : '';
        $auto_time = isset($row['conversion_time']) ? sanitize_text_field($row['conversion_time']) : '';
        $conversion_time = $manual_time !== '' ? $manual_time : ($auto_time !== '' ? $auto_time : get_date_from_gmt($row['created_at_gmt'], 'Y-m-d H:i:s'));
        $override_value = isset($row['conversion_override']) ? sanitize_text_field($row['conversion_override']) : '';
        if ($override_value === '') {
            continue;
        }
    ...
    if (!empty($exported_ids)) {
        $exported_ids = array_values(array_unique(array_filter($exported_ids)));
        foreach ($exported_ids as $submission_id) {
            mdp_set_submission_value($submission_id, 'mdp_conversion_exported', '1');
            mdp_set_submission_value($submission_id, 'mdp_conversion_exported_at', current_time('mysql'));
        }
    }
    exit;
}
...
function mdp_conversions_page_callback() {
    ...
    $selected_form_id = isset($_GET['form_id']) ? sanitize_text_field(wp_unslash($_GET['form_id'])) : '';
    if ($selected_form_id === '') {
        $selected_form_id = mdp_get_last_conversion_form_id(get_current_user_id());
    } else {
        mdp_set_last_conversion_form_id(get_current_user_id(), $selected_form_id);
    }
    if ($selected_form_id === '' && !empty($form_ids)) {
        $selected_form_id = $form_ids[0];
    }
    ...
    $settings = mdp_get_conversion_settings();
    $saved_notice = !empty($_GET['mdp_conversion_saved']);
    $detected_tool = mdp_detect_consent_tool();
    $active_tool = mdp_resolve_consent_tool($settings);
    $tool_labels = mdp_get_consent_tool_labels();
    $field_keys = $selected_form_id !== '' ? mdp_get_form_field_keys($selected_form_id) : [];
    $field_keys = array_values(array_unique(array_filter(array_map('sanitize_key', $field_keys))));
    $entries = $selected_form_id !== '' ? mdp_get_conversion_entries($selected_form_id, 50) : [];
    $script_path = plugin_dir_path(__FILE__) . 'assets/js/conversions.js';
    $script_version = file_exists($script_path) ? (string) filemtime($script_path) : '1.0.0';
    wp_enqueue_script('jquery');
    wp_enqueue_script(
    ...
    wp_add_inline_script('mdp-conversions-admin', 'window.mdpConversionsConfig = ' . wp_json_encode($config) . ';', 'before');
    wp_print_scripts(array('jquery', 'mdp-conversions-admin'));
    ?>
    <div class="wrap mdp-stats-root">
        <h1><?php _e('Google Ads Conversions', 'elementor-forms-statistics'); ?></h1>
        <?php if ($saved_notice) : ?>
            <div class="notice notice-success"><p><?php _e('Conversion-Einstellungen gespeichert.', 'elementor-forms-statistics'); ?></p></div>
        <?php endif; ?>
        <?php if (empty($forms_indexed)) : ?>
            <p><?php _e('Keine Formulare gefunden.', 'elementor-forms-statistics'); ?></p>
        <?php else : ?>
            <div class="mdp-conversions-layout">
                <div class="mdp-conversions-col-left">
                    <div class="mdp-conversions-panel">
                        <h2><?php _e('Formular auswählen', 'elementor-forms-statistics'); ?></h2>
                        <form method="get" action="<?php echo esc_url(admin_url('admin.php')); ?>" class="mdp-conversions-form-select">
                            <input type="hidden" name="page" value="statistiken-conversions">
                            <label for="mdp_conversion_form_id"><?php _e('Formular', 'elementor-forms-statistics'); ?></label>
                            <select id="mdp_conversion_form_id" name="form_id" onchange="this.form.submit();">
                                <?php foreach ($form_ids as $form_id) :
                                    $form_entry = $forms_indexed[$form_id];
                                    $title = mdp_format_form_title($form_id, $forms_indexed, mdp_resolve_form_title($form_id, $form_entry));
                                    $label = $title !== '' ? $title : $form_id;
                    ...
                    <div class="mdp-conversions-panel">
                        <h2><?php _e('Consent-Tool', 'elementor-forms-statistics'); ?></h2>
                        <p class="description">
                            <?php
                            $detected_label = isset($tool_labels[$detected_tool]) ? $tool_labels[$detected_tool] : $detected_tool;
                            printf(esc_html__('Erkannt: %s', 'elementor-forms-statistics'), esc_html($detected_label));
                            ?>
                        </p>
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                            <?php wp_nonce_field('mdp_save_conversion_settings', 'mdp_conversion_settings_nonce'); ?>
                            <input type="hidden" name="action" value="mdp_save_conversion_settings">
                            <input type="hidden" name="form_id" value="<?php echo esc_attr($selected_form_id); ?>">
                            <label for="mdp_consent_tool"><?php _e('Consent-Tool verwenden', 'elementor-forms-statistics'); ?></label>
                            <select id="mdp_consent_tool" name="consent_tool">
                                <?php foreach ($tool_labels as $tool_key => $tool_label) : ?>
                                    <option value="<?php echo esc_attr($tool_key); ?>" <?php selected($settings['consent_tool'], $tool_key); ?>>
                                        <?php echo esc_html($tool_label); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <div class="mdp-conversions-consent-fields mdp-consent-fields" data-tool="ccm19">
                                <label for="mdp_ccm19_category"><?php _e('CCM19 Kategorie', 'elementor-forms-statistics'); ?></label>
                                <input type="text" id="mdp_ccm19_category" name="ccm19_category" value="<?php echo esc_attr($settings['ccm19_category']); ?>" class="regular-text">
                            </div>
                            <div class="mdp-conversions-consent-fields mdp-consent-fields" data-tool="usercentrics">
                                <label for="mdp_usercentrics_category"><?php _e('Usercentrics Kategorie', 'elementor-forms-statistics'); ?></label>
                                <input type="text" id="mdp_usercentrics_category" name="usercentrics_category" value="<?php echo esc_attr($settings['usercentrics_category']); ?>" class="regular-text">
                            </div>
                            <div class="mdp-conversions-consent-fields mdp-consent-fields" data-tool="complianz">
                                <label for="mdp_complianz_category"><?php _e('Complianz Kategorie', 'elementor-forms-statistics'); ?></label>
                                <input type="text" id="mdp_complianz_category" name="complianz_category" value="<?php echo esc_attr($settings['complianz_category']); ?>" class="regular-text">
                            </div>
                            <div class="mdp-conversions-consent-fields">
                                <label for="mdp_conversion_email_key"><?php _e('E-Mail Feld', 'elementor-forms-statistics'); ?></label>
                                <select id="mdp_conversion_email_key" name="email_key">
                                    <?php if (empty($field_keys)) : ?>
                                        <option value=""><?php _e('Keine Formularfelder gefunden.', 'elementor-forms-statistics'); ?></option>
                                    <?php else : ?>
                                        <?php foreach ($field_keys as $field_key) : ?>
                                            <option value="<?php echo esc_attr($field_key); ?>" <?php selected($settings['email_key'], $field_key); ?>>
                                                <?php echo esc_html($field_key); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </select>
                                <label for="mdp_conversion_name_key"><?php _e('Name Feld', 'elementor-forms-statistics'); ?></label>
                                <select id="mdp_conversion_name_key" name="name_key">
                                    <?php if (empty($field_keys)) : ?>
                                        <option value=""><?php _e('Keine Formularfelder gefunden.', 'elementor-forms-statistics'); ?></option>
                                    <?php else : ?>
                                        <?php foreach ($field_keys as $field_key) : ?>
                                            <option value="<?php echo esc_attr($field_key); ?>" <?php selected($settings['name_key'], $field_key); ?>>
                                                <?php echo esc_html($field_key); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                            ...
                            <?php submit_button(__('Einstellungen speichern', 'elementor-forms-statistics'), 'secondary'); ?>
                        </form>
                    </div>
                    <div class="mdp-conversions-panel">
                        <h2><?php _e('Export', 'elementor-forms-statistics'); ?></h2>
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="mdp-export-form">
                            <?php wp_nonce_field('mdp_export_conversions', 'mdp_export_conversions_nonce'); ?>
                            <input type="hidden" name="action" value="mdp_export_conversions_csv">
                            <input type="hidden" name="form_id" id="mdp_conversion_form_id_export" value="<?php echo esc_attr($selected_form_id); ?>">
                            <label for="mdp_conversion_name"><?php _e('Conversion-Name', 'elementor-forms-statistics'); ?></label>
                            <input type="text" id="mdp_conversion_name" name="conversion_name" value="<?php echo esc_attr($form_settings['conversion_name']); ?>" class="regular-text">
                            <label for="mdp_conversion_currency"><?php _e('Währung', 'elementor-forms-statistics'); ?></label>
                            <input type="text" id="mdp_conversion_currency" name="conversion_currency" value="<?php echo esc_attr($form_settings['conversion_currency']); ?>" class="regular-text">
                            <?php submit_button(__('Conversions exportieren', 'elementor-forms-statistics'), 'primary'); ?>
                        </form>
                    </div>
                </div>
                <div class="mdp-conversions-col-right">
                    <div class="mdp-conversions-panel">
                        <h2><?php _e('Einträge', 'elementor-forms-statistics'); ?></h2>
                        <p class="description"><?php _e('Markiere Einträge als Conversion, um sie in den Google Ads Export aufzunehmen.', 'elementor-forms-statistics'); ?></p>
                        <div class="mdp-conversions-table-wrap">
                            <table class="widefat striped mdp-conversions-table">
                                <thead>
                                    <tr>
                                        <th class="check-column"><?php _e('Einreichen', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('ID', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('Datum', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('Name', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('E-Mail', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('GCLID', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('GBRAID', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('WBRAID', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('Auftragswert', 'elementor-forms-statistics'); ?></th>
                                        <th><?php _e('Auftrags Zeitpunkt', 'elementor-forms-statistics'); ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if (empty($entries)) : ?>
                                        <tr>
                                            <td colspan="10"><?php _e('Keine Einträge gefunden.', 'elementor-forms-statistics'); ?></td>
                                        </tr>
                                    <?php else : ?>
                                        <?php foreach ($entries as $entry) :
                                            $conversion_checked = isset($entry['conversion_flag']) && $entry['conversion_flag'] === '1';
                                            $conversion_time_auto = isset($entry['conversion_time']) ? $entry['conversion_time'] : '';
                                            $conversion_time_manual = isset($entry['conversion_time_manual']) ? $entry['conversion_time_manual'] : '';
                                            $conversion_value = isset($entry['conversion_value']) ? $entry['conversion_value'] : '';
                                            $is_exported = isset($entry['conversion_exported']) && $entry['conversion_exported'] === '1';
                                            $created_at = isset($entry['created_at_gmt']) ? mdp_format_submission_created_at($entry['created_at_gmt']) : '';
                                        ?>
                                            <tr class="<?php echo $is_exported ? 'mdp-conversion-exported' : ''; ?>">
                                                <td class="check-column">
                                                    <input type="checkbox"
                                                        class="mdp-conversion-toggle"
                                                        data-submission="<?php echo (int) $entry['ID']; ?>"
                                                        <?php checked($conversion_checked); ?>>
                                                </td>
                                                ...
                                                <td><?php echo esc_html(isset($entry['gclid']) ? $entry['gclid'] : ''); ?></td>
                                                <td><?php echo esc_html(isset($entry['gbraid']) ? $entry['gbraid'] : ''); ?></td>
                                                <td><?php echo esc_html(isset($entry['wbraid']) ? $entry['wbraid'] : ''); ?></td>
                                                <td>
                                                    <input type="text"
                                                        class="mdp-conversion-value"
                                                        data-submission="<?php echo (int) $entry['ID']; ?>"
                                                        value="<?php echo esc_attr($conversion_value); ?>"
                                                        placeholder="">
                                                </td>
                                                <td>
                                                    <?php
                                                    $conversion_time_raw = $conversion_time_manual !== '' ? $conversion_time_manual : $conversion_time_auto;
                                                    $conversion_time_display = $conversion_time_raw;
                                                    if ($conversion_time_display !== '' && strpos($conversion_time_display, 'T') === false) {
                                                        $conversion_time_display = str_replace(' ', 'T', $conversion_time_display);
                                                    }
                                                    ?>
                                                    <input type="datetime-local"
                                                        class="mdp-conversion-time"
                                                        data-submission="<?php echo (int) $entry['ID']; ?>"
                                                        value="<?php echo esc_attr($conversion_time_display); ?>"
                                                        data-default="<?php echo esc_attr($conversion_time_auto !== '' ? str_replace(' ', 'T', $conversion_time_auto) : ''); ?>">
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
